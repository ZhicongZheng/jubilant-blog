      ...
      <ul v-if="articleList.length">
        <li class="search-result" v-for="article in articleList" :key="article.id">
          ...
          <router-link class="search-title" :to="`/article/${article.id}`">
            <span @click="dialogVisible = false" v-html="article.articleTitle" />
          </router-link>
          <!-- 文章内容 -->
          <p class="search-content" v-html="article.articleContent" />
        </li>
      </ul>
      <!-- 搜索结果不存在提示 -->
...
debouncedWatch(keyword, () => (keyword.value ? handleSearch() : (articleList.value = [])), { debounce: 300 })
const handleSearch = () => {
