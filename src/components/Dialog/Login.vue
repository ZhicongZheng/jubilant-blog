<template>
  <n-modal
    ...
    :block-scroll="false"
  >
    <n-input class="mt-11" v-model:value="loginForm.username" placeholder="邮箱号" @keyup.enter="handlelogin" />
    <n-input
      class="mt-11"
    ...
    <div>
      ...
      <div class="social-login-wrapper">
        <svg-icon class="icon" icon-class="qq" size="2rem" v-if="showLogin('qq')" color="#00aaee" />
        <svg-icon class="icon" icon-class="gitee" size="2rem" v-if="showLogin('gitee')" @click="giteeLogin" />
        <svg-icon class="icon" icon-class="github" size="2rem" v-if="showLogin('github')" @click="githubLogin" />
      </div>
    </div>
  </n-modal>
</template>
