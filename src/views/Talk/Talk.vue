<template>
  ...
  <div class="bg">
    <div class="page-container" v-if="talk">
      <div class="talk-item">
        <div class="talk-meta">
          <img class="user-avatar" src="https://static.ttkwsd.top/config/9c65807710f54d9d5ad398a78216ebfb.jpg" />
        </div>
        <div class="talk-content-wrap">
          ...
          <div class="info" style="margin-top: 0.5rem">
            <span class="talk-like info" @click="like">
              <svg-icon icon-class="like" size="0.8rem" :class="isLike(talk.id)" style="margin-right: 5px" />
              {{ talk.likeCount }}
            </span>
            <span class="talk-comment info">
              <svg-icon icon-class="comment" size="0.9rem" style="margin-right: 5px" />
              {{ commentCount == null ? 0 : commentCount }}
            </span>
          </div>
        </div>
      </div>
      <CommentList :comment-type="commentType" @get-comment-count="getCommentCount" />
    </div>
  </div>
</template>
