<template>
  ...
  <div class="bg">
    <div class="page-container">
      <h2><svg-icon class="flower" icon-class="flower" size="1.25rem" color="pink" /> 本站信息</h2>
      <blockquote class="block">
        <p>名称：{{ blog.siteConfig.siteName }}</p>
        <p>简介：{{ blog.siteConfig.siteIntro }}</p>
        <p>头像：{{ blog.siteConfig.authorAvatar }}</p>
      </blockquote>
      <h2><svg-icon class="flower" icon-class="flower" size="1.25rem" color="pink" /> 申请方法</h2>
      <div class="welcome">需要交换友链的可在本页留言 (｡･∀･)ﾉﾞ</div>
      <blockquote class="block">
        <p>出于信息需要,大佬你的信息格式要包含：网站名称、网站链接、头像链接、网站介绍、名称颜色</p>
      </blockquote>
      <h2><svg-icon class="flower" icon-class="flower" size="1.25rem" color="pink" /> 小伙伴们</h2>
      <div class="friends">
        <div class="friend-item" v-animate="['slideUpBigIn']" v-for="friend in friendList" :key="friend.id">
          ...
          <div class="info">
            <a class="name" target="_blank" :href="friend.url" :style="{ color: friend.color }">{{ friend.name }}</a>
